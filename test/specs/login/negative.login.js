describe('Test Sample Login Register APK', () => {
    ...
    it('User input password', async () => {
        ...
        await (input_password).addValue("abc")
    })
    ...
    it('User see alert message', async () => {
        const alert = await $('/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.view.ViewGroup/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.TextView')
        expect (alert).toExist()
        expect (alert).toBeDisplayed()
        expect (alert).toHaveText("Wrong Email or Password")
    })
})